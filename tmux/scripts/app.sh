...
t -g default-terminal "xterm-kitty"
set -g default-terminal "screen-256color" # colors!
tw -g xterm-keys on
t -s escape-time 10                     # faster command sequences
t -sg repeat-time 600                   # increase repeat timeout
t -s focus-events on

t -g prefix2 C-a                        # GNU-Screen compatible prefix
nd C-a send-prefix -2

t -q -g status-utf8 on                  # expect UTF-8 (tmux < 2.2)
tw -q -g utf8 on

t -g history-limit 5000                 # boost history

edit configuration
nd e new-window -n '~/.tmux.conf.local' "sh -c '\${EDITOR:-vim} ~/.tmux.conf.local && tmux source ~/.tmux.conf && tmux display \"~/.tmux.conf sourced\"'"

reload configuration
nd r run 'tmux source ~/.tmux.conf'


-- display -------------------------------------------------------------------

t -g base-index 1           # start windows numbering at 1
tw -g pane-base-index 1     # make pane numbering consistent with windows

tw -g automatic-rename on   # rename window to reflect current program
t -g renumber-windows on    # renumber windows when a window is closed

t -g set-titles on          # set terminal title

t -g display-panes-time 800 # slightly longer pane indicators display time
t -g display-time 1000      # slightly longer status messages display time

t -g status-interval 10     # redraw status line every 10 seconds

clear both screen and history
nd -n C-l send-keys C-l \; run 'sleep 0.1' \; clear-history

activity
t -g monitor-activity on
t -g visual-activity off
...
find session
nd C-f command-prompt -p find-session 'switch-client -t %%'
...
pane navigation
nd -r h select-pane -L  # move left
nd -r j select-pane -D  # move down
nd -r k select-pane -U  # move up
nd -r l select-pane -R  # move right
nd > swap-pane -D       # swap current pane with the next one
nd < swap-pane -U       # swap current pane with the previous one

maximize current pane
nd + run 'cut -c3- ~/.tmux.conf | sh -s _maximize_pane "#{session_name}" #D'

pane resizing
nd -r H resize-pane -L 2
nd -r J resize-pane -D 2
nd -r K resize-pane -U 2
nd -r L resize-pane -R 2

window navigation
bind n
bind p
nd -r C-h previous-window # select previous window
nd -r C-l next-window     # select next window
nd Tab last-window        # move to last active window
...
nd U run "cut -c3- ~/.tmux.conf | sh -s _urlview #{pane_id}"


-- facebook pathpicker -------------------------------------------------------

nd F run "cut -c3- ~/.tmux.conf | sh -s _fpp #{pane_id}"


-- list choice (tmux < 2.4) --------------------------------------------------

vi-choice is gone in tmux >= 2.4
n -b 'tmux bind -t vi-choice h tree-collapse 2> /dev/null || true'
n -b 'tmux bind -t vi-choice l tree-expand 2> /dev/null || true'
n -b 'tmux bind -t vi-choice K start-of-list 2> /dev/null || true'
n -b 'tmux bind -t vi-choice J end-of-list 2> /dev/null || true'
n -b 'tmux bind -t vi-choice H tree-collapse-all 2> /dev/null || true'
n -b 'tmux bind -t vi-choice L tree-expand-all 2> /dev/null || true'
n -b 'tmux bind -t vi-choice Escape cancel 2> /dev/null || true'


-- edit mode (tmux < 2.4) ----------------------------------------------------

vi-edit is gone in tmux >= 2.4
n -b 'tmux bind -ct vi-edit H start-of-line 2> /dev/null || true'
n -b 'tmux bind -ct vi-edit L end-of-line 2> /dev/null || true'
n -b 'tmux bind -ct vi-edit q cancel 2> /dev/null || true'
n -b 'tmux bind -ct vi-edit Escape cancel 2> /dev/null || true'


-- copy mode -----------------------------------------------------------------

nd Enter copy-mode # enter copy mode
t-window-option -g mode-keys vi
nd-key -T copy-mode-vi v send -X begin-selection
nd-key -T copy-mode-vi y send-keys -X copy-pipe-and-cancel "pbcopy"
nd-key -T copy-mode-vi MouseDragEnd1Pane send-keys -X copy-pipe-and-cancel "pbcopy"
nd-key -T copy-mode-vi Enter send-keys -X copy-pipe-and-cancel "pbcopy"
...
nd b list-buffers  # list paste buffers
nd p paste-buffer  # paste from the top paste buffer
nd P choose-buffer # choose which buffer to paste from


-- user defined overrides ----------------------------------------------------

 '[ -f ~/.tmux.conf.local ]' 'source ~/.tmux.conf.local'


-- 8< ------------------------------------------------------------------------

n 'cut -c3- ~/.tmux.conf | sh -s _apply_configuration'

Splitting
nd "|" split-window -h -c "#{pane_current_path}" -l 1 "~/.dotfiles/scripts/tmux-animate.sh -x 0.5 -p; /bin/zsh"
nd "\\" split-window -h -c "#{pane_current_path}" -l 1 "~/.dotfiles/scripts/tmux-animate.sh -x 0.3 -p; /bin/zsh"
nd "-" split-window -v -c "#{pane_current_path}" -l 1 "~/.dotfiles/scripts/tmux-animate.sh -y 0.5 -p; /bin/zsh"
nd _ split-window -v -c "#{pane_current_path}" -l 1 "~/.dotfiles/scripts/tmux-animate.sh -y 0.3 -p; /bin/zsh"
...
_apply_theme() {
  ...
  tmux_conf_theme_message_fg=${tmux_conf_theme_message_fg:-'#282828'}   # black
  ...
  tmux_conf_theme_message_command_bg=${tmux_conf_theme_message_command_bg:-'#282828'} # black
  ...
  tmux_conf_theme_mode_fg=${tmux_conf_theme_mode_fg:-'#282828'} # black
  ...
  tmux_conf_theme_window_status_current_fg=${tmux_conf_theme_window_status_current_fg:-'#282828'} # black
  ...
  tmux_conf_theme_status_left_fg=${tmux_conf_theme_status_left_fg:-'#282828,#e4e4e4,#e4e4e4'}  # black, white , white
...
EOF
  ...
  tmux_conf_theme_status_right_fg=${tmux_conf_theme_status_right_fg:-'#8a8a8a,#e4e4e4,#282828'} # light gray, white, black
